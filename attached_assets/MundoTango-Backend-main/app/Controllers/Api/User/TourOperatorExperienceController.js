...
class TourOperatorExperienceController extends RestController {

    constructor() {
        super('TourOperatorExperience')
        this.resource = 'TourOperatorExperience';
    ...
    async addTourOperatorExperience({ request, response }) {
        try {
            ...
            // form status -> 11
            await User.instance().updateUserFormStatus(11, request?.user?.id)
    ...
    async getTourOperatorExperience({request, response}) {
        try {
            this.request = request;
            this.response = response;
...
module.exports = TourOperatorExperienceController