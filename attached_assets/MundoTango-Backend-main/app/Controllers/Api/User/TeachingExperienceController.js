...
class TeachingExperienceController extends RestController {

    constructor() {
        super('TeachingExperience')
        this.resource = 'TeachingExperience';
    ...
    async addTeachingExperience({ request, response }) {
        try {
            ...
            if(!exist) {
                return this.sendError(
                    ...
                    "Please provide Teacher Experience date.",
                    400
                )
            }
            ...
            // form status -> 4
            await User.instance().updateUserFormStatus(4, request?.user?.id)
    ...
    async getTeachingExperience({request, response}) {
        try {
            this.request = request;
            this.response = response;
...
module.exports = TeachingExperienceController