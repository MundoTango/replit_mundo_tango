...
class NonTangoActivityController extends RestController {

    constructor() {
        super('NonTangoActivity')
        this.resource = 'NonTangoActivity';
    ...
    async addNonTangoActivity({request, response}) {
        try {
            // this.__collection = false
            ...
            let recordCreate = await this.modal.createRecord({
                parent_id: Number(request?.body?.parent_id) || null,
                name: request?.body?.name || null,
                icon_url: image_url || null,
            })
            this.__is_paginate = false;
            // this.__collection = false
...
module.exports = NonTangoActivityController