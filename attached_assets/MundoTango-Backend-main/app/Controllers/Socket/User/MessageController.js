...
class MessageController extends SocketRestController {
    ...
    async beforeStoreLoadModel() {
        ...
        if (params.chat_room_slug) {
            ...
            console.log("Find Room Record : ", record)
            if (_.isEmpty(record)) {
                ...
                console.log(record.slug, "chat_room_slug")
            }
            else {
    ...
    async afterStoreLoadModel(record) {
        console.log("record after create:", record)
        this.socket.body.message_slug = record.slug;
        console.log(this.socket.body, "after msg sent")
        ...
        if (this.socket.body.is_newRoomMessage) {
            this.resource = 'ChatMessages'
            console.log('user_' + this.socket.body.target_user_slug, 'user_' + this.socket.user.slug)
            ...
            console.log('room_' + this.socket.body.chat_room_slug)
    ...
    async deleteChatMessage({ io, socket }) {
        ...
        const message_record = await this.modal.getRecordByCondition(this.socket, { slug: params.message_slug, deletedAt: null });
