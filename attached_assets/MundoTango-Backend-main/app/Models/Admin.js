...
class Admin extends User {
    ...
    async getDashboardData(request) {
        let users_count = await this.orm.findAndCountAll({
            where: {
                user_type: ROLES.USER,
                deletedAt: null
            }
        })
        let total_active_posts = await Post.instance().orm.findAndCountAll({
            where: {status: 'active', deletedAt: null}
        })
        let total_groups = await Group.instance().orm.findAndCountAll({
            where: {deletedAt: null}
        })
        let total_events = await Event.instance().orm.findAndCountAll({
            where: {deletedAt: null}
        })

        const monthly_revenue_data = await UserSubscription.instance().orm.findAll({
            attributes: [
                [sequelize.fn("DATE_FORMAT", sequelize.col("expiry_date"), "%Y-%m"), "month"],
                [sequelize.fn("SUM", sequelize.col("amount")), "totalRevenue"], // Summing the `amount` field
            ],
            where: {
                status: 'active', // Optional: Include only 'active' subscriptions in the revenue calculation
            },
            group: [sequelize.fn("DATE_FORMAT", sequelize.col("expiry_date"), "%Y-%m")],
        ...
        return {
            ...
            total_active_posts: total_active_posts?.count,
