...
const { API_TOKENS_ENUM } = require("../../../config/enum.js");
...
class UserOTPController extends RestController {
    ...
    async beforeStoreLoadModel() {
        ...
        let user = await User.instance().getUserByEmail(params.email);
        if (_.isEmpty(user)) {
    ...
    async verifyOTPRegister({ request, response }) {
        ...
        const record = await this.modal.verifyOTP(request, params)
        ...
        await this.modal.deleteRecord(user.email)
