...
const AboutTabContent = () => {
  ...
  const id = getUserId();
  const userid = id ? id : userEmail?.id;
  console.log(userid)
  const {
    data: userAbout,
    isLoading: getAboutLoading,
    refetch: getAboutRefetch,
  } = useGetReviewDetailsQuery(userid, {
    skip: !userid,
  });
  const [getTrangoActivities, { data: trongoactivity, isLoading: getLoading }] =
    useGetTrangoActivityMutation();
  ...
  const marks = [
    // {
    //   value: 0,
    //   label: <div className="mt-2 text-xs">0</div>,
    // },
    {
      value: 1,
  ...
  useEffect(() => {
    if (!getAboutLoading && userAbout?.data) {
      setAboutDetail(userAbout?.data);
      if (!aboutDetail) {
        reset(userAbout?.data);
      }
    }
  }, [userAbout, getAboutLoading, reset]);

  useEffect(() => {
    if (userid) {
      getAboutRefetch();
    }
  }, [userid, getAboutRefetch]);
  ...
  useEffect(() => {
    if (aboutDetail) {
      ...
      setdance_role_follower(aboutDetail?.user_questions?.dance_role_follower);
      setValue(
        "dance_role_leader",
      ...
      setValue("language", aboutDetail?.user_questions?.languages?.split(","));
      ...
      let record = [];
      for (let i = 0; i < 1; i++) {
        const { city, property_url, space, people } = aboutDetail?.housing_host;
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      ...
      if (organizerExperienceChanged) {
        apiCalls.push(addOrganizerExperience(apiPayload.organizer_experience));
      }
      let profile_link = [
    ...
    <div className="bg-background-color h-screen overflow-y-scroll">
      ...
      <div className="md:mx-28 md:my-14">
        ...
        <div className="mt-5 flex justify-between">
          ...
          >
            {loading ? <SpinnerLoading /> : <NextArrow />}
          </button>
        </div>
      </div>
    </div>
