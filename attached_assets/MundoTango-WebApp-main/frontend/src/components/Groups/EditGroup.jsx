...
import { useNonTangoActivityMutation } from "@/data/services/activityApi";
import {
  useAddEventMutation,
...
const EditModal = ({
  ...
  const [updateGroup, { isLoading: groupLoading }] = useUpdateGroupMutation();
  const [updateEvent, { isLoading: eventLoading }] = useUpdateEventMutation();
  ...
  const onSubmit = async (record) => {
    if (type === "group") {
      try {
        ...
        formData.append("group_type", record?.group_type);
        formData.append("name", record?.name);
        formData.append("description", record?.about);
        formData.append("latitude", latitude);
        formData.append("longitude", longitude);
        formData.append("members", []);
        formData.append("privacy", record?.privacy);
      ...
      try {
        ...
        formData.append("latitude", latitude);
        formData.append("longitude", longitude);
        formData.append("privacy", record?.privacy || data?.privacy);
        formData.append("media", bannerImageFile);
        formData.append(
          "event_type_id",
        ...
        formData.append("start_date", record?.startDate);
        formData.append("end_date", record?.endDate);
        formData.append("location", location);
        formData.append("visibility", record?.visibility);
        formData.append("about_space", record?.about_space);
  ...
  useEffect(() => {
    ...
    setValue("about", data?.description);
    setValue("privacy", data?.privacy);
    setValue("group_type", data?.group_type);
    setBannerImage(data?.image_url);
    setValue("startDate", formatDate(data?.start_date));
    setValue("endDate", formatDate(data?.end_date));
    setValue("startTime", formatTime(data?.start_date));
    setValue("endTime", formatTime(data?.end_date));
    setValue("about_space", data?.about_space);
    setCity(data?.city);
    setCountry(data?.country);
    setValue("visibility", data?.visibility);
    setValue("event_type_id", data?.event_type_id);

    const perticiapant =
      data?.event_participants?.map((x) => {
        return {
          id: x?.user?.id,
          ...
          <form className="relative" onSubmit={handleSubmit(onSubmit)}>
                    ...
                    render={({ field }) => (
                      <div className="">
                        ...
                        >
                          <Autocomplete
                            onLoad={(autocomplete) =>
                              (autoEventRef.current = autocomplete)
                            }
                                ...
                                onClick={(e) => {
                                  ...
                                  setAddOptions((prev) =>
                                    prev.filter(
                                      (item) =>
                                        item?.id !== user?.id ||
                                        item?.memberType !== user?.memberType
                                    )
                                  );
            ...
            <div className="flex gap-2 pt-2 w-full">
              ...
              <button
                ...
                disabled={eventLoading || groupLoading}
              >
