...
function TimeLine({ myprofile, event_id }) {
  ...
  const [commentLike, { isLoading: commentPostingLoading }] =
    useCommentLikeMutation();
  ...
  const getAllPost = async () => {
    console.log("get called", event_id);
    try {
      let temp = [...postList];
              ...
              postList?.map((item, index) => (
                <PostLikeComment
                  ...
                  handleOpenFriendModal={() => {
                    handleOpenFriendModal();
                    setFriendId(
                      item.is_shared ? item?.shared_by_user?.id : item?.user_id
                    );
                  }}
                  onShareOpenPopup={() => {
                    setShareModal(true);
