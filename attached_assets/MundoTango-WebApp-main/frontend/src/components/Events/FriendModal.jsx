...
const FriendModal = ({ user_slug, handleClose, openmodal, setUserSlug, button_title, modal_name , isCloseNeed = true,  onClickButton , data , loading, btnclass, btn_loading }) => {
                ...
                (item, index) => (
                  ...
                  >
                    <div className="flex gap-3">
                      <img
                        src={item?.friend?.user_images?.length > 0 ? item?.friend?.user_images[0]?.image_url : ''}
                        alt=""
                      ...
                      <div className="text-[#323232]">
                        ...
                        <div className="text-xs font-bold">@{item?.friend?.username}</div>
                      </div>
                    </div>
                    <div>
                      <button
                        onClick={() => {onClickButton(item?.friend?.id); isCloseNeed && handleClose()}}
                        className={`${btnclass} ${item?.friend?.is_in_event == 3 ? "bg-gray-text-color" : "bg-btn-color"}  w-full px-10 py-2 text-xs font-medium text-white rounded-lg`}
                        disabled={item?.friend?.is_in_event == 3}
                      >
                        {btn_loading ? <SpinnerLoading size={10}/> : item?.friend?.is_in_event == 3 ? `Invited` : button_title || 'Add'}
                      </button>
                    </div>
                  </div>
                )
