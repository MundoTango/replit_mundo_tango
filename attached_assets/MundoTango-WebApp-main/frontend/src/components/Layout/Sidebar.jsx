...
import { useGetMundoTangoDetailQuery } from "@/data/services/tangoDetails";
...
const Sidebar = ({ isOpen, setIsOpen, toggleSidebar, handleCloseSearch }) => {
  ...
  let { data, isLoading } = useGetMundoTangoDetailQuery();
  ...
  const GROUPS = [
    ...
    },
    {
      ...
      count: data?.dancer_city_count,
    },
  ];

  const [confirmPopup, setConfirmPopup] = useState(false);

  const handlePopOpen = () => setConfirmPopup(true);

  const handlePopClose = () => setConfirmPopup(false);
          ...
          {SIDEBAR_ROUTES.map(({ icon, title, link }, index) => (
            <Link legacyBehavior href={link} key={index}>
              <div className="py-2 cursor-pointer select-none">
                ...
                >
                  <div
                    className={`border-r-[6px] rounded-r-md h-9  transition-all group-hover:border-btn-color  ${_isActive(link) ? "border-btn-color" : "border-white"}`}
                  />
