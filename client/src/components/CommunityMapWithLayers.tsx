...
export default function CommunityMapWithLayers({
  ...
  const mapItems: MapItem[] = [
    ...
    // Events
    ...events
      .filter((event: any) => event?.lat != null && event?.lng != null)
      .map((event: any) => ({
        id: event.id,
    ...
    // Host homes
    ...homes
      .filter((home: any) => home?.lat != null && home?.lng != null)
      .map((home: any) => ({
        id: home.id,
        lat: Number(home.lat),
        lng: Number(home.lng),
        ...
        type: 'home' as const,
        city: home.city || '',
        price: home.pricePerNight || 0,
        photos: home.photos || [],
      })),
    // Recommendations
    ...recommendations
      .filter((rec: any) => rec?.lat != null && rec?.lng != null)
      .map((rec: any) => ({
      id: rec.id,
      lat: rec.lat,
      lng: rec.lng,
    ...
    if (item.type === 'cityGroup') {
      ...
      return (
        <div className="relative p-0 min-w-[320px] max-w-[360px] bg-gradient-to-br from-turquoise-500/10 via-cyan-500/10 to-blue-500/10 backdrop-blur-lg rounded-xl border border-turquoise-300/30 shadow-2xl overflow-hidden">
          {/* ESA LIFE CEO 56x21 - MT Ocean Theme glassmorphic design */}
          ...
          <div className="relative p-5">
            ...
            {/* ESA LIFE CEO 56x21 - Statistics Grid with glassmorphic cards */}
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-white/40 backdrop-blur-md rounded-xl p-3 border border-white/20 shadow-lg hover:bg-white/50 transition-all group">
                <div className="flex items-center gap-2">
                  <div className="p-2 bg-gradient-to-br from-blue-500 to-indigo-500 rounded-lg shadow-md group-hover:scale-110 transition-transform">
            ...
            {/* View Community Button with glassmorphic style */}
            <button 
              className="w-full mt-4 bg-gradient-to-r from-turquoise-600 to-cyan-600 text-white py-2.5 px-4 rounded-xl font-semibold text-sm hover:from-turquoise-700 hover:to-cyan-700 transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
              onClick={() => {
            ...
            >
              View {item.city} Community
            </button>
            
            {/* Direction buttons with glassmorphic style */}
            <div className="flex gap-2 mt-3">
              <a 
                href={getGoogleMapsUrl(item.lat, item.lng, item.title)}
                target="_blank"
                rel="noopener noreferrer"
                className="flex-1 flex items-center justify-center gap-1.5 bg-white/40 backdrop-blur-sm hover:bg-white/60 text-gray-800 py-2 px-3 rounded-lg text-xs font-medium transition-all border border-white/20"
              >
  ...
  return (
    <div className="h-full w-full relative">
      ...
      >
        ...
        {mapItems
          .filter(item => item.type === 'cityGroup' && item.lat != null && item.lng != null)
          .map(item => {
            return (
              <Marker
                key={`city-${item.id}`}
